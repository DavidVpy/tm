...
async function imprimirTicketBluetooth(datos) {
  ...
  if (datos.tipo === 'pago') {
    ...
    addCenter(sep);
    sp();
    add('FECHA: ' + fmtFecha(datos.fechaPago || new Date()));
    add('CLIENTE: ' + mayus(datos.nombreCliente));
    add('PEDIDO: ' + mayus(datos.numeroPedido));
    if (datos.productoDesc) add('PRODUCTO: ' + mayus(datos.productoDesc));
    add(sep);
    add('CUOTA: ' + datos.numeroCuota + '/' + datos.totalCuotas, 1);
    add('MONTO RECIBIDO:', 1);
    addCenter(fmtGs(datos.montoPago), 1, 2);
    ...
    addCenter('COMPROBANTE DE VENTA', 1, 1);
    addCenter(sep);
    sp();
    add('FECHA: ' + fmtFecha(datos.fechaVenta || new Date()));
    add('CLIENTE: ' + mayus(datos.nombreCliente));
    add('PEDIDO: ' + mayus(datos.numeroPedido));
    add(sep);
    add('PRODUCTOS:', 1);
    (datos.productos||[]).forEach(p => {
      ...
      add('  ' + p.cantidad + ' X ' + fmtGs(p.precio) + ' = ' + fmtGs(p.cantidad * p.precio));
    });
    add(sep);
    add('TOTAL: ' + fmtGs(datos.totalVenta), 1);
    if (parseFloat(datos.entregaInicial) > 0) {
      add('ENTREGA INICIAL: ' + fmtGs(datos.entregaInicial));
      add('SALDO A FINANCIAR: ' + fmtGs(datos.saldoFinanciar));
    }
    if (datos.cantidadCuotas > 0) {
      add('PLAN: ' + datos.cantidadCuotas + ' CUOTAS DE ' + fmtGs(datos.valorCuota));
      if (datos.fechaPrimeraCuota) add('1RA CUOTA: ' + fmtFecha(datos.fechaPrimeraCuota));
    }
    add('GARANTIA: ' + (datos.garantiaMeses||0) + ' MESES');
  ...
  // Guardar en Apps Script y construir URL para Thermer
...
async function reimprimirComprobante(idVenta) {
  ...
  await imprimirTicketBluetooth({
    ...
    totalVenta: venta.Total_Venta, entregaInicial: venta.Entrega_Inicial,
    saldoFinanciar: venta.Saldo_Financiar, cantidadCuotas: venta.Cantidad_Cuotas,
    valorCuota: venta.Saldo_Financiar/(venta.Cantidad_Cuotas||1),
    garantiaMeses: venta.Garantia_Meses, vendedor: APP.user.nombre
  });
}
...
async function confirmarAnularPago(idPago) {
  ...
  hideLoader();
  if (result.success) {
    mostrarConfirmacion('PAGO ANULADO');
    setTimeout(()=>{ const mv = document.getElementById('modalDetalleVenta'); if(mv) { const id = mv.querySelector('[id]')?.id; mv.remove(); } },1900);
  } else toast(result.error||'ERROR AL ANULAR','error',4000);
}
...
document.getElementById('formVenta').addEventListener('submit', async function(e){
  ...
  if (result.success) {
    ...
    setTimeout(async()=>{
      await imprimirTicketBluetooth({
        tipo:'venta', nombreCliente:datos.nombreCliente, numeroPedido:result.numeroPedido,
        productos:datos.productos, totalVenta:datos.totalVenta, entregaInicial:datos.entregaInicial,
        saldoFinanciar:result.saldoFinanciar, cantidadCuotas:datos.cantidadCuotas,
        valorCuota:result.valorCuota, garantiaMeses:datos.garantiaMeses,
        fechaPrimeraCuota:result.fechaPrimeraCuota, vendedor:datos.vendedor, fechaVenta:datos.fechaVenta
      });
...
async function cargarCuotasCliente() {
  ...
  if (!idCliente) { document.getElementById('cuotasCliente').innerHTML=''; return; }
  document.getElementById('cuotasCliente').innerHTML='<div class="loading">CARGANDO CUOTAS...</div>';
  const cuotas = await getCuotasClienteAPI(idCliente);
  if (!cuotas.length) { document.getElementById('cuotasCliente').innerHTML='<div class="empty">SIN CUOTAS</div>'; return; }
  let html='';
  cuotas.forEach(c=>{
...
function mostrarModalPago(idCuota, saldo, idVenta, numCuota, totalCuotas) {
  ...
  modal.innerHTML = `<div style="background:white;border-radius:24px 24px 0 0;padding:24px;width:100%;max-width:500px;">
    ...
    <div style="margin-bottom:12px;">
      <label style="font-weight:600;display:block;margin-bottom:4px;font-size:14px;">CUOTA ${numCuota}/${totalCuotas}</label>
      ...
      <input type="number" id="montoPagoInput" value="${saldo}" step="1000" style="width:100%;padding:14px;border:2px solid var(--border);border-radius:12px;font-size:16px;box-sizing:border-box;">
    </div>
    <div style="margin-bottom:16px;">
      <label style="font-size:13px;color:var(--muted);display:block;margin-bottom:4px;">FECHA DEL PAGO</label>
      <input type="date" id="fechaPagoInput" value="${fmtDateInput(new Date())}" style="width:100%;padding:14px;border:2px solid var(--border);border-radius:12px;font-size:15px;box-sizing:border-box;">
    </div>
...
async function confirmarPago(idCuota, idVenta, saldo) {
  ...
  const montoPago = parseFloat(input?input.value:0);
  const fechaPago = fechaInput?fechaInput.value:fmtDateInput(new Date());
  if (!montoPago||montoPago<=0) { toast('INGRESÁ UN MONTO VÁLIDO','warning'); return; }
  if (montoPago>saldo+1) { toast('EL MONTO NO PUEDE SUPERAR EL SALDO','error'); return; }
  ...
  hideLoader();
  if (result.success) {
    mostrarConfirmacion('PAGO REGISTRADO');
    setTimeout(async()=>{
      await imprimirTicketBluetooth({
        tipo:'pago', nombreCliente:datos.nombreCliente, numeroPedido:idVenta,
        codigoVerif:result.codigoVerificacion, montoPago:montoPago,
        saldoRestante:result.saldoRestante, proximaFecha:result.proximaFecha,
        productoDesc:result.productoDesc||'', numeroCuota:result.numeroCuota,
        totalCuotas:result.totalCuotas, cobrador:APP.user.nombre, fechaPago
      });
      setTimeout(()=>cargarCuotasCliente(),3000);
    },1900);
